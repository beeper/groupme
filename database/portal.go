...
func ParsePortalKey(inp string) *PortalKey {
	...
	if len(parts) == 1 {
		...
		return &PortalKey{types.NewGroupMeID(inp), types.NewGroupMeID(inp)}
	} else if len(parts) == 2 {
		...
		return &PortalKey{
			GMID:     types.NewGroupMeID(parts[1]),
			Receiver: types.NewGroupMeID(parts[0]),
		}
	} else {
		return nil
	}
...
func GroupPortalKey(gmid types.GroupMeID) PortalKey {
	return PortalKey{
		GMID:     gmid,
		Receiver: gmid,
	}
...
func NewPortalKey(gmid, receiver types.GroupMeID) PortalKey {
	return PortalKey{
		GMID:     gmid,
		Receiver: receiver,
	}
...
func (pq *PortalQuery) GetAllByGMID(gmid types.GroupMeID) []*Portal {
